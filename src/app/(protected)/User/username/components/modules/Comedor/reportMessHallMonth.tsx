...
export function ReporteComedorMensual() {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.post(
        "/api/Alumnos/Comedor/reportMessHallMonth",
        params
      );
      ...
      <div className="p-4 bg-amber-50 rounded-lg shadow-sm shadow-amber-700">
        ...
        <div className="mb-6">
          <input
            ...
            className="p-2 border-4 border-amber-300 rounded-lg w-full focus:outline-none 
           focus:ring-2 focus:ring-amber-600 focus:border-transparent transition-all uppercase"
            value={search}
        ...
        <div className="flex flex-wrap items-end gap-3 mb-6">
          ...
          <div className="w-auto h-10">
            <button
              onClick={fetchData}
              disabled={loading}
              className="h-full px-4 py-2 bg-gradient-to-r from-amber-700 to-amber-600 
             text-white rounded-md shadow hover:from-amber-600 
             hover:to-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500 disabled:opacity-70"
            >
              {loading ? (
