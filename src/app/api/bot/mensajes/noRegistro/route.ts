...
export async function enviarNotificacionTelegram(params: NotificacionParams) {
  try {
    ...
    if (telegramResult.recordset && telegramResult.recordset.length > 0) {
      ...
      const mensaje = `🚨 *Salida No Registrada*

Estimado padre o tutor, le informamos que su hijo(a) ${nombre} ${paterno}, del semestre ${semestre}, *no ha registrado su salida* del plantel el día *${fecha}*.

Le sugerimos confirmar esta situación directamente con el alumno.

Este mensaje ha sido generado automáticamente por el sistema de control de asistencia.`;
